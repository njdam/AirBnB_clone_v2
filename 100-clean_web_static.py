...
from os import path, mkdir, stat, unlink, listdir
...
def do_clean(number=0):
    """A function that Deletes all unnecessary archives."""
    ...
    if begin < len(archives):
        archives = archives[begin:]
    else:
        archives = []
    for archive in archives:
        unlink('versions/{}'.format(archive))
    # Command to delete all archives except recent released two archives
    cmd = [
        "rm -rf $(",
